...
export const getAllJobs = catchAsyncError(async (req, res, next) => {
  ...
  export const postJob = catchAsyncError(async (req, res, next) => {
    ...
    export const getAllJobs = catchAsyncError(async(req, res, next) =>{
       const {city, nich,  searchKeyword} = req.query;
       const query = {}
       if(city){
          query.location = city
       }
...
{/*
    export const postJob = catchAsyncError(async (req, res, next) =>{
        const {title, jobType, location, companyName, introducation, responsibilities, qualifications, offers, 
            salary, hiringMultipleCandidates, personalWebsiteTitle, personalWebsiteUrl,jobNiche, newLettersSent} = req.body
            if(!title|| !jobType|| !location|| !companyName|| !introducation|| !responsibilities|| !qualifications|| 
                !salary|| !jobNiche){
             return next(new ErrorHandler("Please provide full job details.",400 ))
            }
