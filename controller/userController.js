//import { catchAsyncErrors } from "../middlerwares/catchAsyncError.js";
...
{/*
  export const updateProfile = catchAsyncError(async (req, res, next) => {
      const newUserData = {
          ...
          niches: {
            firstNiche: req.body.firstNiche,
            secoundNiche: req.body.secoundNiche,
            thiredNiche: req.body.thiredNiche,
          },
      };
  
      const { firstNiche, secoundNiche, thiredNiche} =  newUserData.niches;
  
      if(req.user.role == "Job Seeker" && (!firstNiche || !secoundNiche || !thiredNiche))
      {
        return next (
          new ErrorHandler("Please provide your all preferred job niches.", 400)
        );
      ...
      if(req.files){
        ...
        if(resume){
          ...
          newUserData.resume = {
            public_id: newUserData.public_id,
            url: newResume.secure_url,
          };
        }
      }
